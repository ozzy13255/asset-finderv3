...
Deno.serve(async(req)=>{
 ...
 try{
  ...
  const body=await req.json(); const {name,employee_number,login_code,password,role='user',patch_id}=body; const email=String(login_code||'').trim().toLowerCase()
  if(!name||!employee_number||!email||!password||!patch_id)return json({error:'name, employee number, email, password and patch are required'},400)
  if(!emailRe.test(email))return json({error:'Login & Recovery Email must be a valid email address.'},400)
  ...
  const {data:created,error:createError}=await admin.auth.admin.createUser({email,password,email_confirm:true}); if(createError)return json({error:createError.message},400)
  const userRole=actor.role==='owner'?role:'user'
  const {error:profileError}=await admin.from('profiles').insert({id:created.user.id,name,employee_number,login_code:email,role:userRole,status:'active',must_change_password:true,recovery_email:email})
  ...
  return json({id:created.user.id,name,employee_number,login_code:email,role:userRole,patch_id})
