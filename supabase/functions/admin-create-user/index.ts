...
Deno.serve(async (req)=>{
  ...
  try {
    ...
    const email=String(login_code||'').trim().toLowerCase()
    if(!name||!employee_number||!email||!password||!patch_id) return json({error:'name, employee_number, email, password and patch_id are required'},400)
